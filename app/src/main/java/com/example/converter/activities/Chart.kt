...
class Chart : AppCompatActivity() {
    ...
    private fun init(){
        ...
        dataObjects.add(ChartModel(2.6f,8f))
        dataObjects.add(ChartModel(2.68f,9f))
        dataObjects.add(ChartModel(2.86f,10f))
        dataObjects.add(ChartModel(2.86f,11f))
        dataObjects.add(ChartModel(2.86f,12f))
        var entries: ArrayList<Entry>  = ArrayList()
        dataObjects.forEach {
            entries.add(Entry(it.x, it.y))
        }
        var dataSet = LineDataSet(entries, "USD -> GEL")
